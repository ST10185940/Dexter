...
using DxtrEncryption;
// using DxtrHealthCheck; - soon (b4 the file compression remember)
...
class Dexter
{
    ...
    internal class Password(string PasswordName, string payload)
    {
        [Required]
        private protected string Name { get; set; } = PasswordName;

        [Required]
        private protected string Hash { get; set; } = payload;
    }
    ...
    private static string GeneratePassword(int length , int strength, List<string> options )
    {
        ...
        string numbers = numbersSelected == null ? "" : "0123456789๑๒๓๔๕๖๗๘๙๑๐一二三四五六七八九十";  
    ...
    private static string GetArgon2Hash(string password, byte[] salt)
    {
    ...
    private static string GetSCryptHash(string password, byte[] saltNPepper)
    {
    ...
    private static async void PasswordGen()
    {
         try{
            ...
            List<string> strengthOptions = [
                    "1. Absurdly Strong (!WARNING: Anything stronger may not usable)",
                    "2. Even Mightier",
                    "3. Dexter's Special Brew"
                ];
            ...
            var options = AnsiConsole.Prompt(
                ...
                .AddChoices([
                        ...
                        "Exclude Visually Ambiguous Characters", "Exclude duplicate Characters",
    ...
    private static async Task SubMenu(string password){
        var option = AnsiConsole.Prompt(
            new SelectionPrompt<string>()
            .Title("Password Menu")
            .AddChoices([
                "1. Generate Another Password",
                "2. Save Previously Generated Password",
                "3. Main Menu",
                "4. Exit"
            ]));
    ...
    private static async Task SaveAync(string hash){
       try{
            Type("Name the password");
            string? name = Console.ReadLine();

            Password saved = new(name,hash);

            var path = "Dexter/yourPasswords.csv";
            ...
            else
            {
                using (var writer = new StreamWriter(path))
                {
                    ...
                    await writer.WriteLineAsync($"{saved}");
                }
                Type("Password saved");
            }
        }
    ...
    private static void Menu()
    {
        ...
        var option = AnsiConsole.Prompt(
            ...
            switch(string.Format(option)){
                ...
                PasswordGen();
                break;
                case "2. File Encryption/Decryption": break;
